...
def process(filename: str, replacements: Dict[str, str], output: str = None):
    parser = argparse.ArgumentParser(description='Replace text in a PDF file.')
   
    # if length of replacements is 0, return error
    if not replacements:
        ...
        if isinstance(contents, pypdf.generic._data_structures.ArrayObject):
            for content in contents:
                for key, value in replacements.items():
                    total_replacements += replace_text(content, charmaps, key, value)
        elif isinstance(contents, pypdf.generic._data_structures.ContentStream):
            for key, value in replacements.items():
                total_replacements += replace_text(contents, charmaps, key, value)
        else:
